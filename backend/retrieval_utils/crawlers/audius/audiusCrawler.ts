...
export class AudiusCrawler {
    ...
    for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        ...
        if (error.response?.status === 404) {
          throw new CrawlerError('Page not found', url, 404, error);
        }
  ...
  async getSitemapUrls(): Promise<string[]> {
    try {
      ...
      throw new CrawlerError('Failed to parse sitemap', undefined, undefined, error);
    }
  }
  ...
  async processPage(url: string): Promise<ProcessedPage> {
    try {
      ...
      if (error instanceof CrawlerError) throw error;
      throw new CrawlerError('Failed to process page', url, undefined, error);
    }
  }
} 